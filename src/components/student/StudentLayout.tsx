...
export default function StudentLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    async function checkStudent() {
      ...
      if (!user || (user.user_metadata?.role !== 'student' && user.user_metadata?.role !== 'admin')) {
  ...
  if (isStudent === null) return <div style={{ background: 'var(--background)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--foreground)' }}>Loading...</div>;

  const menuItems = [
    { name: 'My Dashboard', href: '/student', icon: '🏠' },
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--background)', color: 'var(--foreground)' }}>
      ...
      <div style={{
        ...
        <div style={{ 
          ...
          <div style={{ fontWeight: 800, fontSize: '1rem' }}>STUDENT <span className="gradient-text">PORTAL</span></div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <span style={{ fontSize: '0.8rem', color: 'var(--muted)' }}>Student</span>
            <button
